...
class CaptchaSecurityImages { 
  ...
  function CaptchaSecurityImages($width = 105,$height = 27, $characters = 4) { 
    $code = $this->generateCode($characters); 
    $font_size = $height * 0.60; 
    $image = @imagecreate($width, $height) or die('Cannot initialize new GD image stream'); 
 
    /* set the colours */ 
    $bgR = mt_rand(204, 255); $bgG = mt_rand(204, 255); $bgB = mt_rand(204, 255); 
  ...
  /*   for($i = 0; $i < ($width*$height) / 3; $i++) { 
   ...
   /*  for($i = 0; $i < ($width*$height) / 150; $i++) { 
    ...
    imagefilledpolygon($image, $values, 4, $clr); 
...
$captcha = new CaptchaSecurityImages(105, 27, rand(4, 4)); 
